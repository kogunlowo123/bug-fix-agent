"""Application configuration for Bug Fix Agent."""
...
class Settings(BaseSettings):
    ...
    app_name: str = Field(default="bug-fix-agent", description="Application name")
    ...
    database_url: str = Field(
        default="postgresql+asyncpg://postgres:postgres@localhost:5432/bug_fix_agent",
    ...
    opensearch_index: str = Field(default="bug_fix_agent_vectors", description="Index")
    ...
    mcp_server_name: str = Field(default="bug-fix-agent-mcp", description="MCP server name")

    a2a_discovery_url: str = Field(default="http://localhost:8500", description="A2A discovery URL")
    a2a_agent_id: str = Field(default="bug-fix-agent", description="A2A agent ID")
